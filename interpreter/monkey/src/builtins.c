...
static struct object builtin_list(const struct object_list* args);
static struct object builtin_narg(const struct object_list* args);
static struct object builtin_args(const struct object_list* args);
...
const struct {
    ...
    { "list", MAKE_BUILTIN(builtin_list) },
    { "narg", MAKE_BUILTIN(builtin_narg) },
    { "args", MAKE_BUILTIN(builtin_args) },
...
if (args->size != 1) {
    ...
    if (args->values[0].type != OBJ_STRING) {
        return make_error_object("invalid argument: expected string, got %s", object_type_to_str(args->values[0].type));
    }

    const char *path = args->values[0].value.ptr->value;

    FL_DIR dirstat;
    if (!fl_opendir(path, &dirstat)) {
        return make_error_object("failed to open directory: %s", path);
    }
...
builtin_narg(const struct object_list* args) {
    //struct object arg = args->values[0];
    return make_integer_object(44);
}
static struct object 
builtin_args(const struct object_list* args) {
    struct object_list *list = make_object_list(2);
    char *str[6]={"1","2","3","4","5","6"};
    for (int i = 0; i < 6; ++i)
    {
        list = append_to_object_list(list, make_string_object(str[i], NULL));
    }
